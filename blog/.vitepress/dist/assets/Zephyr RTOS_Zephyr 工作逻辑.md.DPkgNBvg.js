import{_ as a,o as n,c as i,ak as p}from"./chunks/framework.DS_P0xx8.js";const o=JSON.parse('{"title":"Zephyr 工作逻辑","description":"","frontmatter":{},"headers":[],"relativePath":"Zephyr RTOS/Zephyr 工作逻辑.md","filePath":"Zephyr RTOS/Zephyr 工作逻辑.md"}'),e={name:"Zephyr RTOS/Zephyr 工作逻辑.md"};function l(t,s,h,c,r,k){return n(),i("div",null,[...s[0]||(s[0]=[p(`<h1 id="zephyr-工作逻辑" tabindex="-1">Zephyr 工作逻辑 <a class="header-anchor" href="#zephyr-工作逻辑" aria-label="Permalink to “Zephyr 工作逻辑”">​</a></h1><p>本文介绍了针对某一特定板卡下 zephyr 的工作逻辑，包括从 arch 层的适配到上层 core 属性的调用。</p><h2 id="逐层覆盖结构" tabindex="-1">逐层覆盖结构 <a class="header-anchor" href="#逐层覆盖结构" aria-label="Permalink to “逐层覆盖结构”">​</a></h2><p>zephyr 利用设备树进行外设管理，且这些外设的配置是独立于业务代码的。这使得在合适的配置下， zephyr 可以保证在更改运行平台时，不更改业务代码的任何部分。zephyr 将外设分为以下层级：</p><ul><li>Architecture</li><li>SoC</li><li>Boards</li><li>Custom parts</li></ul><h3 id="architecture" tabindex="-1">Architecture <a class="header-anchor" href="#architecture" aria-label="Permalink to “Architecture”">​</a></h3><p>首先，zephyr 社区对于目前主流的架构进行了适配。其在 <code>zephyr/arch</code> 目录下提供了对于如中断处理、上下文切换、内存管理等架构相关的驱动代码。通常情况下，开发者不需要关注这部分代码的内部细节。笔者将在后续的文档中说明这部分代码的组成。这是外设层级的第一层（最底层），利用 C 语言和汇编语言编写。</p><h3 id="soc" tabindex="-1">SoC <a class="header-anchor" href="#soc" aria-label="Permalink to “SoC”">​</a></h3><p>其次，芯片厂商和 zephyr 社区对其设计生产的芯片进行 SoC 层的适配，体现为提供了一系列 C 语言头文件/源文件供下一级驱动文件（drivers）等调用，同时提供了 <code>dtsi</code> 设备树文件，为 boards 等下一级设备树文件调用。下列代码为节选自 <code>stm32f411.dtsi</code> ，即 ST 公司为其 stm32f411 系列芯片外设进行的硬件描述。一般情况下，开发者也不需要关注这部分代码的内部细节，因为其在芯片厂商的参考手册中有所对应。</p><div class="language-dts"><button title="Copy Code" class="copy"></button><span class="lang">dts</span><pre class="shiki shiki-themes github-light github-dark" style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#24292e;" tabindex="0" dir="ltr"><code><span class="line"><span>/ {</span></span>
