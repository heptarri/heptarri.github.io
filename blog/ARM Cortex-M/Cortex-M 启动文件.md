...
在 ARM Cortex-M 体系中，启动文件通常是使用汇编（`.s` 文件）编写的。其在编译时被编译器编译为 `.o` 目标文件。接下来，链接器（Linker）根据分散加载文件将其放到镜像地址的最开头，作为芯片上电复位后首先运行的部分。
...
其中 `g_pfnVectors` 为中断向量表，`Default_Handler` 为默认的中断处理函数。
...
> R0 R1 R2 R3 是 ARM 的 CPU 内部的通用寄存器，读写速度极快，一般用于逻辑处理。对于 Cortex-M 共有 15 个，从 R0 到 R12，一个为 4 字节宽。根据冯诺依曼架构，CPU 不可以直接从 Flash/RAM 读取数据，所以需要通用寄存器作为中转。 除此之外，还有 R13(SP) 堆栈指针，R14(LR) 链接寄存器存储函数跳回的地址，R15(PC) 程序计数器存储 CPU 当前执行的代码行。
