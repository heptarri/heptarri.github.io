...
RTAB-Map（Real-Time Appearance-Based Mapping）是一种开源 SLAM 算法库。其基于外观的闭环检测机制使其可以实现利用 RGB-D/立体相机及雷达生成环境的 3D 点云或者 2D 栅格地图。

本文利用 Stereolabs 旗下 ZED2i 双目摄像头为平台运行 RTAB-Map 算法进行建图。
...
这是第三方“鱼香 ROS”提供的安装脚本。选择 ROS2 Jazzy 即可。

> 本文环境为 Ubuntu 24.04 + CUDA 13，NVIDIA 驱动版本 580。

然后从 Stereolabs 官方网站下载 ZED SDK：

https://www.stereolabs.com/en-sg/developers/release

> 截至本文发布，其更新至 ZED SDK for Ubuntu 24 5.2（CUDA13 - TensorRT 10） 
...
ls $(ros2 pkg prefix zed_wrapper)/share/zed_wrapper/launch/
```
若有输出，则证明安装完成：
...
zed_tutorial_video
zed_wrapper
/home/hep/ros2_ws/install/zed_wrapper
__pycache__  zed_camera.launch.py
```
## 运行

可通过以下命令运行 zed_wrapper。

```bash
ros2 launch zed_wrapper zed_camera.launch.py camera_model:=zed2i
```
使用以下命令查看 zed_wrapper 发送的主题：

```bash
ros2 topic list
```

可以使用以下命令查看 zed_wrapper 发送的主题可以合成的点云和其他数据：

```bash
ros2 launch zed_display_rviz2 display_zed_cam.launch.py camera_model:=zed2i
```

需要注意的是，zed_wrapper 默认不会启用很多主题的发送，需要在 config 中手动开启，配置文件的位置为 `~/ros2_ws/src/zed-ros2-wrapper/zed_wrapper/config/common_stereo.yaml`。
...
需要注意的是，其中各种 `topic` 的名称是根据 `ros2 topic list` 的结果引用的。在不同环境下可能需要更换。

```bash
hep@hep-pc:~/ros2_ws$ ros2 topic list
    # ...
...
/**:
    ros__parameters:
        ...
        video:
            ...
            publish_left_right: true # Advertise the left and right image topics that are published only if a node subscribes to them
            publish_raw: true # Advertise the raw image topics that are published only if a node subscribes to them
            publish_gray: true # Advertise the gray image topics that are published only if a node subscribes to them
            publish_stereo: true # Advertise the stereo image topic that is published only if a node subscribes to it
            # Other parameters are defined, according to the camera model, in the 'zed.yaml', 'zedm.yaml', 'zed2.yaml', 'zed2i.yaml'
            # 'zedx.yaml', 'zedxmini.yaml', 'virtual.yaml' files
        ...
        depth:
            ...
            publish_depth_info: true # Advertise the depth info topic that is published only if a node subscribes to it
